...
# 改枪码正则（实测码约24位，以6开头，字母+数字）
BUILD_CODE_RE = re.compile(r'\b6[A-Z0-9]{21,27}\b')

# ============ 全武器列表（名称已按游戏内正式名）============
# category_id: 1突击 2冲锋 3狙击/射手 4战斗步枪 5轻机枪 6霰弹 7手枪
WEAPONS = [
    # ---- 突击步枪 ----
...
def write_to_supabase(items):
    ...
    try:
        ...
        if r.status_code in (200, 201, 204):
            ...
            print(f"⚠️ 写库返回 {r.status_code}: {r.text[:80]}")
...
def crawl():
    ...
    results, found, not_found = [], 0, []

    for w in WEAPONS:
        name, keyword = w["name"], w["keyword"]
